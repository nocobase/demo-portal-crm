...
function ActivityList() {
  ...
  const typeOptions = useMemo(
    () =>
      ACTIVITY_TYPES.map((type) => ({ value: type.value, label: type.label })),
    []
  );
    ...
    return [
      ...
      columnHelper.accessor("type", {
        ...
        cell: ({ getValue }) => {
          ...
          return <EnumBadge value={value} label={labelFor(ACTIVITY_TYPES, value)} />;
        },
      }),
      ...
      columnHelper.accessor((record) => record.contact?.name, {
        id: "contactName",
        header: translate("crm.activities.fields.contact", { ns: "app" }, "Contact"),
        enableSorting: false,
        cell: ({ getValue }) => getValue() || "—",
      }),
      columnHelper.display({
        id: "actions",
        header: translate("crm.common.actions", { ns: "app" }, "Actions"),
        enableSorting: false,
        size: 96,
        cell: ({ row }) => (
          <div className="flex items-center gap-1">
  ...
  }, [customerOptions, locale, typeOptions, translate]);
